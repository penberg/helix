...
public:
    nordic_itch_session(const std::vector<std::string>& symbols, core::ob_callback process_ob, core::trade_callback process_trade);

    virtual void process_packet(const char *buf, size_t size) override;
};

class nordic_itch_protocol : public core::protocol {
    virtual nordic_itch_session* new_session(const std::vector<std::string>&, core::ob_callback process_ob, core::trade_callback process_trade) override;
};
